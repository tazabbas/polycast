...
export default function TranscribePage() {
  ...
  async function handleUpload() {
    ...
    try {
      ...
      if (data.transcript) {
        ...
        const saveRes = await fetch('/api/transcriptions', {
          method: 'POST',
        ...
        if (saveData.transcription?.id) {
          setTranscriptionId(saveData.transcription.id)
        }
      } else {
  ...
  return (
    <main style={{ padding: '2rem', fontFamily: 'sans-serif', maxWidth: '680px' }}>
      ...
      <p style={{ fontSize: '0.9rem', color: '#666', marginBottom: '1.5rem' }}>Upload a video or audio file to transcribe it with AI, then translate into any language.</p>
        ...
        <>
          ...
          <div style={{ background: '#f5f5f5', padding: '1.5rem', borderRadius: '12px', marginBottom: '1.5rem' }}>
            ...
            <select
              ...
              style={{ padding: '0.5rem 1rem', borderRadius: '8px', border: '1px solid #ddd', fontSize: '0.9rem', marginBottom: '1rem', display: 'block', width: '100%', maxWidth: '300px' }}
            >
              {LANGUAGES.map((lang) => (
            ...
            <button
              ...
              style={{ background: translating ? '#ccc' : '#533AB7', color: 'white', border: 'none', padding: '0.75rem 1.5rem', borderRadius: '8px', fontSize: '0.95rem', cursor: translating ? 'not-allowed' : 'pointer' }}
            >
          ...
          {translatedText && (
            <div style={{ background: '#fff', border: '1px solid #eee', borderRadius: '12px', padding: '1.5rem' }}>
              ...
              <p style={{ fontSize: '0.95rem', lineHeight: 1.7, margin: 0 }}>{translatedText}</p>
            </div>
          )}
        </>
      )}
